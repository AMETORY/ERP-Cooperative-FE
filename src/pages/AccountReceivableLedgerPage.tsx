...
import { getAccountReceivableLedger } from "../services/api/reportApi";
import toast from "react-hot-toast";
import { LedgerReport } from "../models/report";
import { money } from "../utils/helper";
import { useNavigate } from "react-router-dom";

interface AccountReceivableLedgerPageProps {}

const AccountReceivableLedgerPage: FC<
  AccountReceivableLedgerPageProps
> = ({}) => {
  ...
  const generateReport = (isDownload: boolean) => {
    ...
    getAccountReceivableLedger(data)
      .then((res: any) => {
        if (isDownload) {
      ...
      <div className="p-6 h-[calc(100vh-65px)] flex flex-row">
        <div className="flex flex-grow flex-col px-4 ">
          <h1 className="text-3xl text-gray-900 font-bold">
            {t("accounts_receivable_subsidiary_ledger")}
          </h1>
                  ...
                  <tbody>
                    ...
                    <tr className="border border-gray-400">
                      ...
                      <td className="p-2 border border-gray-300">
                        Batas Kredit{" "}
                        <span className="font-semibold ml-2">
                          {money(report?.contact?.debt_limit)}
                        </span>
                      </td>
                      <td className="p-2 border border-gray-300">
                       Sisa Batas Kredit{" "}
                        <span className="font-semibold ml-2">
                          {money(report?.contact?.debt_limit_remain)}
                        </span>
                      </td>
                    </tr>
                  </tbody>
                    ...
                    <tbody className="text-center">
                      ...
                      {report.ledgers.map((ledger) => (
                        <tr key={ledger.id}>
                          ...
                          <td className="border border-gray-300 p-2 hover:font-semibold hover:underline cursor-pointer" onClick={() => nav(`/sales/${ledger.ref_id}`)}>
                            {ledger.ref}
                          </td>
                          <td className="border border-gray-300 p-2">
...
export default AccountReceivableLedgerPage;
